...
from units import Conv2d, Reduction_A
...
class DeepPrint(nn.Module):
    def __init__(self, in_channels=3, num_classes=5000, k=192, l=224, m=256, n=384):
        super(DeepPrint, self).__init__()
        ...
        Stem_block.append(Stem(in_channels))
        ...
        # Inception A (6x)
        for i in range(6):
            domain_A_blocks.append(Inception_A(384))
        

        # stem
        self.stem = nn.Sequential(*Stem_block)


        # texture branch
        self.features = nn.Sequential(*blocks)
        self.global_texture_average_pooling = nn.AdaptiveAvgPool2d((1, 1))


        # domain branch
        self.domain_features = nn.Sequential(*domain_A_blocks)

        # D(x)
        self.D_x = nn.Sequential(
            nn.ConvTranspose2d(384, 384, 3, stride=2, padding=1, bias=False),
            nn.ReLU(True),
            Conv2d(384, 128, 7, stride=1,  padding=1, bias=False),
            nn.ConvTranspose2d(128, 128, 3, stride=2, padding=1, bias=False),
            nn.ReLU(True),
            Conv2d(128, 32, 3, stride=1, padding=0, bias=False),
            Conv2d(32, 6, 1, stride=2, padding=0, bias=False),
        )

        self.M_x = nn.Sequential(
            Conv2d(384, 384, 3, stride=1,  padding=1, bias=False),
            Conv2d(384, 768, 3, stride=2,  padding=0, bias=False),
            Conv2d(768, 768, 3, stride=1,  padding=1, bias=False),
            Conv2d(768, 896, 3, stride=2, padding=0, bias=False),
            Conv2d(896, 1024, 3, stride=1, padding=0, bias=False),
            nn.AdaptiveAvgPool2d((1,1)),
        )

        self.dropout = nn.Dropout(0.8)
        self.linear = nn.Linear(1536, 96)
        self.relu_1 = nn.ReLU(True)
        self.linear_2 = nn.Linear(1024, 96)
        self.relu_2 = nn.ReLU(True)
        self.linear_3 = nn.Linear(96, num_classes)
        self.linear_4 = nn.Linear(96, num_classes)
    ...
    def forward(self, x):
        ...
        texture_output = self.features(x) # ([2, 1536, 12, 12])
        texture_output = self.global_texture_average_pooling(texture_output)  # ([2, 1536, 1, 1])
        texture_output = texture_output.view(texture_output.size(0), -1)
        texture_output = self.dropout(texture_output)
        texture_output = self.linear(texture_output) # output 96 dimension

        #domain
        domain_output = self.domain_features(x)
        D_output = self.D_x(domain_output)
        # D_output = D_output.permute(0, 2, 3, 1)

        M_output = self.M_x(domain_output)
        M_output = M_output.view(M_output.size(0), -1)
        M_output = self.dropout(M_output)
        M_output = self.linear_2(M_output) # output 96 dimension
        
        Representation = torch.cat((texture_output,M_output),dim=1) # output 192 dimension

        texture_class_output_1 = self.linear_3(texture_output) # class
        M_class_output_2 = self.linear_4(M_output) # class
        
        return Representation,texture_output, M_output, D_output, aligned_img, CNN_feature_map
...
if __name__=="__main__":
    ...
    net = DeepPrint().cuda()
    print(net(a)[0].shape,net(a)[1].shape,net(a)[2].shape)
