...
class CenterLoss(nn.Module):
    ...
    def __init__(self, num_classes: int, feat_dim: int, alpha: float = 0.01):
        ...
        self.register_buffer(
            "centers",
            torch.nn.functional.normalize(torch.randn(num_classes, feat_dim).cuda(), dim=1),
            persistent=True,
        )
        self.counter = 0
        self.nupdate = 0
    ...
    def forward(self, x: torch.Tensor, labels: torch.LongTensor):
        ...
        with torch.no_grad():
            batch_centers = torch.index_select(self.centers.cuda(), 0, labels.cuda())
        # Get difference of x from batch centers
        diff = (x - batch_centers).cuda()
        # Update current centers
        with torch.no_grad():
            self.centers.index_add_(0, labels.cuda(), diff, alpha=self.alpha)
